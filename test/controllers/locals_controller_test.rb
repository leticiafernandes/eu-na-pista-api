...
class LocalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post locals_url, params: { local: { comp_text: @local.comp_text, lat: @local.lat, lng: @local.lng, local_text: @local.local_text } }
    end
  ...
  test "should update local" do
    patch local_url(@local), params: { local: { comp_text: @local.comp_text, lat: @local.lat, lng: @local.lng, local_text: @local.local_text } }
